...
def order_summary(request):
   ...
   total_price=0
   cart_items=Cart.objects.filter(uid=request.user.id)
   dataa['products'] = cart_items
   dataa['cart_items']=cart_items_count(request)
   #getting cart items based on quantity
   for item in cart_items:
      total_items+=item.quantity
      total_price+=(item.quantity*item.pid.price)
   dataa['total_items']=total_items
   dataa['total_price']=total_price
   ...
   return render(request,'user/order_summary.html',context=dataa)
